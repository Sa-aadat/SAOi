...
!----------------------------------------------------------------------!
      ...
      double precision dtheta1, dtheta2
!
      !stop ' Analytical gradients have not been implemented - helical' 
!
      x1 = x(1)
      x2 = x(2)
      x3 = x(3)
...
!      
      if (x1.gt.0.d0) then
!          
       gf(1) = x1*1.d0/dsqrt(x1**2+x2**2)
     &       * (dsqrt(x1**2+x2**2)-1.d0)*2.0d2
     &       + (1.d0/x1**2*x2*(x3-(datan(x2/x1)*5.d0)/pi)*1.0d3)
     &       / (pi*(1.d0/x1**2*x2**2+1.d0))
       gf(2) = x2*1.d0/dsqrt(x1**2+x2**2)
     &       * (dsqrt(x1**2+x2**2)-1.d0)*2.0d2
     &       - (x3*1.0d3-(datan(x2/x1)*5.0d3)/pi)
     &       / (pi*x1*(1.d0/x1**2*x2**2+1.d0))
       gf(3) = x3*2.02d2-(datan(x2/x1)*1.0d3)/pi;  
!    
      elseif (x1.lt.0.d0) then
!       
       gf(1) = x1*1.d0/dsqrt(x1**2+x2**2)
     &       * (dsqrt(x1**2+x2**2)-1.d0)*2.0d2
     &       + (1.d0/x1**2*x2*(x3-(pi*5.d0
     &       + datan(x2/x1)*5.d0)/pi)*1.0d3)
     &       / (pi*(1.d0/x1**2*x2**2+1.d0))
       gf(2) =x2*1.d0/dsqrt(x1**2+x2**2)
     &       * (dsqrt(x1**2+x2**2)-1.d0)*2.0d2
     &       - (x3*1.0d3-(pi*5.0d3+datan(x2/x1)*5.0d3)/pi)
     &       / (pi*x1*(1.d0/x1**2*x2**2+1.d0))
       gf(3) = x3*2.02d2-(pi*1.0d3+datan(x2/x1)*1.0d3)/pi
!        
      else 
!        
       gf(1) = x1*1.d0/dsqrt(x1**2+x2**2)
