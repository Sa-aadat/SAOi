...
!----------------------------------------------------------------------!
      ...
      double precision xidb, xidh, sbar
!
      do k=1,ni*n
        gc(k)=0.d0
      enddo
...
!
      do i=1,nseg
        ...
        i1=2*i-1
        k=(i1-1)*ni+j1
        gc(k)=(12.d0*pload*xleni**2)/(2.d0*young*xidb)*a1/ybar
        i1=2*i
        k=(i1-1)*ni+j1
        gc(k)=(12.d0*pload*xleni**2)/(2.d0*young*xidh)*a1/ybar
      enddo
!
      do i=1,nseg-1
        xidb=-(x(2*i-1)**2*x(2*i)**3)
        ...
        i1=2*i-1
        k=(i1-1)*ni+j1
        gc(k)=gc(k)+(12.d0*pload*xleni**2)/(young*xidb)*
     &                 b1*(nseg-i)/ybar
        i1=2*i
        k=(i1-1)*ni+j1
        gc(k)=gc(k)+(12.d0*pload*xleni**2)/(young*xidh)*
     &                 b1*(nseg-i)/ybar
      enddo
!
      return
